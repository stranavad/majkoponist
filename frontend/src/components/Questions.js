...
class Questions extends Component {
  ...
  countDown = () => {
    ...
    if (this.state.seconds <= 0) {
      ...
      if (seconds <= 0) {
        this.setState({
          answeredQuestions: this.state.answeredQuestions.concat({
            id: this.state.questions[this.state.question_number].id,
            answer: "",
          }),
        });
  ...
  render() {
    ...
    if (this.state.display_question) {
      component = (
        <Question
          question={this.state.questions[this.state.question_number]}
          question_number={this.state.question_number + 1}
          onSubmit={this.answerQuestion}
          time_left={this.state.seconds}
        />
      );
    } else {
      component = <PlayScreen playButton={this.startTimer} />;
    }
    return <div className="centerbox">{component}</div>;
  }
}
